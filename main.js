...
module.exports.loop = function () {
  ...
  //  Game.spawns.Spawn1.room.createConstructionSite(containerFlag.x, containerFlag.y + 1, STRUCTURE_CONTAINER);
    ...
    if(supplyTower.length < 1 && towers.length > 0){
        if(towers[0].energyAvailable() != towers[0].store.energyCapacity())
    ...
    if(Object.keys(Game.creeps).length == 0)
        ...
        var newName = 'Harvester' + Game.time;
        console.log('Spawning new harvester: ' + newName);
        currentSpawn.spawnCreep([WORK,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE], newName,
    ...
    else if(conRepair.length < 1)
    {
      ...
      currentSpawn.spawnCreep([WORK,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE], newName,
          {memory: {
              role: 'energyMover',
              container: ''
          }});
    ...
    else if(currentRoom.energyAvailable >= 900 && upgraders2.length < 6) {
        var newName = 'Upgrader2' + Game.time;
